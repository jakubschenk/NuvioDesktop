...
private fun StreamLaunchContent(
    ...
    var resolvingDebridStream by rememberSaveable(
        launch.videoId,
        launch.parentMetaId,
        launch.seasonNumber,
        launch.episodeNumber,
    ...
    fun cacheKeyForResolvedVideo(): String =
    ...
    fun cacheStreamLink(stream: StreamItem, sourceUrl: String) {
        if (!playerSettings.streamReuseLastLinkEnabled) return
        StreamLinkCacheRepository.save(
            contentKey = cacheKeyForResolvedVideo(),
            url = sourceUrl,
    ...
    fun launchPlayerFromStream(
        ...
        cacheStreamLink(stream, sourceUrl)
        onPlayerLaunch(
            buildPlayerLaunch(
                stream = stream,
                sourceUrl = sourceUrl,
    ...
    LaunchedEffect(effectiveVideoId, hasResolvedVideoId, playerSettings.streamReuseLastLinkEnabled, launch.manualSelection) {
        ...
        val cacheKey = cacheKeyForResolvedVideo()
        val maxAgeMs = maxOf(
            3L * 60L * 1000L,
    ...
    LaunchedEffect(
        ...
        val launched = launchPlayerFromStream(
            stream = stream,
            sourceUrl = sourceUrl,
...
private fun MainAppContent(
    ...
    onEditProfiles: () -> Unit = {},
        ...
        var searchFocusRequestCount by remember { mutableIntStateOf(0) }
    ...
    LaunchedEffect(
        ...
        fun launchPlaybackWithDownloadPreference(
            ...
            openStreams: (StreamLaunch) -> Unit = { launch ->
            ...
            SharedTransitionLayout {
                                    ...
                                    key(selectedAppLanguage.code) {
                                        NuvioNavigationBar {
                                            ...
                                            NavItem(
                                                ...
                                                onClick = { handleRootTabClick(AppScreenTab.Discover) },
                        ...
                        sourceContent = { onBackToEpisodeSelector ->
                            detailStreamLaunch?.let { launch ->
                                StreamLaunchContent(
                                    ...
                                    onPlayerLaunch = { playerLaunch, _, forceExternal, forceInternal ->
                                        StreamsRepository.cancelLoading()
                                        if (!forceInternal && (forceExternal || playerSettingsUiState.externalPlayerEnabled)) {
                                            openExternalPlayback(playerLaunch)
                                        } else {
                ...
                composable<StreamRoute> { backStackEntry ->
                    ...
                    StreamLaunchContent(
                        ...
                        onPlayerLaunch = { playerLaunch, replacePicker, forceExternal, forceInternal ->
                            StreamsRepository.cancelLoading()
                            if (!forceInternal && (forceExternal || playerSettingsUiState.externalPlayerEnabled)) {
                                openExternalPlayback(playerLaunch)
                            } else {
