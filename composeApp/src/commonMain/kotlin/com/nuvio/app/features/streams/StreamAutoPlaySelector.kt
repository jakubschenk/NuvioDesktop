    ...
    fun evaluateAutoPlayStream(
        ...
        val preferredProviderStream = if (
            ...
            candidateStreams.firstOrNull { stream ->
                stream.isAutoPlayable(debridEnabled, activeResolverProviderId) &&
                    (
                        (targetAddonId.isNotEmpty() && stream.addonId == targetAddonId) ||
                            (targetAddonName.isNotEmpty() && stream.addonName == targetAddonName)
                        )
            }
        } else {
            null
        }
        val preferredStream = preferredReadyStream ?: preferredProviderStream
        ...
        return StreamAutoPlayEvaluation(
            readyStreams = readyStreams,
            hasPendingDebridCandidate =
                matchingStreams.any {
                    it.isPendingDebridAutoPlay(debridEnabled, activeResolverProviderId)
                } ||
                    (preferCurrentProviderInSelection &&
                        (targetAddonId.isNotEmpty() || targetAddonName.isNotEmpty()) &&
                        candidateStreams.any { stream ->
                            ((targetAddonId.isNotEmpty() && stream.addonId == targetAddonId) ||
                                (targetAddonName.isNotEmpty() && stream.addonName == targetAddonName)) &&
                                stream.isPendingDebridAutoPlay(debridEnabled, activeResolverProviderId)
                        }),
        )
    }
